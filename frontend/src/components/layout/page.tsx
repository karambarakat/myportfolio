...
export const CommonLayout = component$(({ project }: { project: Project }) => {
  return (
    <div>
      ...
      <div class="rounded-xl pb-5">
        <DisplayImage src={project.displayPicture?.src} />
      </div>
      <h1 class="typo-h2 pb-0">{project.title}</h1>
      <p class="typo-dim">
        created at{" "}
        <time>
          {DateTime.fromISO(project.publishedAt).toFormat("LLL dd yyyy")}
        </time>
      </p>
